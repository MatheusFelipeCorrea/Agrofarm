      ...
      if (!textoAssistente) {
        ...
        if (ia.ok && ia.texto) {
          ...
          textoAssistente = construirRespostaSomenteDados(
            ctx,
            conteudo,
            `A chave no .env parece incorreta (${tam ? `${tam} caracteres` : "formato inválido"}; use a chave copiada de https://aistudio.google.com/apikey — costuma começar com AIza ou AQ.). Reinicie a API.`,
          );
